...
var updateMessage = {
    roundResult: function (round) {
        return `Round ${round.round}: You ${
            round.roundResult
        } that round. Your choice: ${round.playerChoice}, enemy choice: ${
            round.aiChoice
        }. Result after round: ${round.resultAfterRound}`;
    },
    gameWin: function (name, result) {
        return `Congratulation ${name}, you won the game! ${result.wins} : ${
            result.losses
        } for you. `;
    },
    gameLoose: function (name, result) {
        return `Sorry ${name}, you loose the game! ${result.losses} : ${
            result.wins
        } for computer.`;
    },
    gameDraw: function () {
